$(function () {
  ...
  $("button[name=check-now]").on("click", function () {
    ...
    $.cookie(
      "link_checker_selector",
      $("#link-checker-check-now .Inputfield_selector code").text()
    );
    $iframe.addClass("loading").attr("src", $iframe.data("src"));
  });
  $("#link-checker-check-now").on(
    "finished",
    ".link-crawler-summary",
  ...
  $("body")
    .on("click", ".AdminDataTable .edit-comment", function () {
      ...
      if (comment !== null) {
        $.post(
          moduleConfig.processPage + "comment",
          { links_id: link, comment: comment },
          function (data) {
    ...
    .on("click", ".AdminDataTable .remove-link", function () {
      ...
      var link = $(this).data("link");
      $.post(
        moduleConfig.processPage + "remove",
        { links_id: link },
        function (data) {
    ...
    .on("change", ".AdminDataTable input[name=skip]", function () {
      ...
      $.post(
        moduleConfig.processPage + "skip",
        { links_id: link, skip: skip },
        function () {
          var $table = $(
            "table." +
              (skip
                ? "skipped-links"
                : $container.find(".status-4xx").length
                ? "broken-links"
                : "redirects")
          );
          $container.fadeOut("fast", function () {
  ...
  if ($("ul.Inputfields:not(ul.Inputfields ul.Inputfields)").length) {
    $("ul.Inputfields:not(ul.Inputfields ul.Inputfields)").WireTabs({
      items: $(
        "ul.Inputfields:not(ul.Inputfields ul.Inputfields) > .Inputfield"
      ),
  ...
  $(
    "a[href^=#link-checker]:not(:first):not(a[href^=#link-checker-check-now])"
  ).on("click", function () {
    ...
    if (!tables[table]) {
      ...
      $.get(location.pathname + "table", { id: table }, function (data) {
        ...
        if (data) {
          var $filters = $(
            '<form class="link-checker-filters" data-table="' + table + '">'
          );
          var $filters_status = $('<select name="status">');
          $filters_status.append(
            $("<option>").attr("value", "").text(moduleConfig.i18n.filterStatus)
          );
          var statuses = [];
